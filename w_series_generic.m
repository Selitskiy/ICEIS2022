...
[X, Ys, Y, B, XI, C, k_ob] = w_series_generic_train_tensors(M, m_in, n_out, l_sess, n_sess, norm_fl);
...
for i = 1:n_sess
    ...
    if exist('regNet') == 0
        ...
        %regNet = makeGMDHNet(i, m_in, n_out, k_hid1, k_hid2, 0.07, 0.04, 5, mb_size, X, Y);
...
if ~strcmp(modelName,'lstm')
    [X2, Y2s, Y2, Yh2, Bt, k_tob] = w_series_generic_test_tensors(M, m_in, n_out, l_sess, l_test, t_sess, sess_off, offset, norm_fl, 0);
else
